// modified from js-event-bus https://www.npmjs.com/package/js-event-bus
(function (caller, bus) {
  ...
  var EventBus = function () {
    ...
    return {
      ...
      removeListener: function (eventName, index) {
				if(that.indexToBeRemoved == null)
					that.indexToBeRemoved = [];

				that.indexToBeRemoved.push({eventName:eventName,index:index});
      },
      /**
       * Remove all the events
       */
      getCurrentTarget: function() {
				return that.currentTarget;
      },
      ...
      emit: function (eventName, context) {
        ...
        listeners.forEach(function (info, index) {
				...
				// remove unused target listener
				if(that.indexToBeRemoved != null){
					for(let tmp of that.indexToBeRemoved){
						delete that.listeners[tmp.eventName][tmp.index];
					}
				}

				that.indexToBeRemoved = null;
      }
    };
  };
