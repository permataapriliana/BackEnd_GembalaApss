...
class commonHelper {
	...
	// change to must format timestamp ex 12:00:00
	hmsToMs(hms){
	...
	addTolerance(value, tolerance){
		const self = this;
		const dateStart = date.isValid(value, 'YYYY-MM-DD HH:mm:ss') ? new Date(value):value;

		const start = date.addMinutes(dateStart, tolerance);
		const startWithTolerance = self.formatDate(start)

		return startWithTolerance
	}
}
